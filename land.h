...
#define LOGO_TEXTURE_HEIGHT 128
...
void initLandscapePrimitives(LandscapeScene *buffer)
{
	...
	SetDrawTPage(&buffer->logoLeftTPage,0,0,GetTPage(1,0,LOGO_TEXTURE_X_POS,0));
	SetSprt(&buffer->logoLeftSprite);
	setUV0(&buffer->logoLeftSprite, 0, 0);
	setWH(&buffer->logoLeftSprite,256,LOGO_TEXTURE_HEIGHT);
	setXY0(&buffer->logoLeftSprite,0,0);
	SetShadeTex(&buffer->logoLeftSprite, 1); // Set texture shading to off
	buffer->logoLeftSprite.clut = GetClut(LOGO_TEXTURE_X_POS,LOGO_TEXTURE_HEIGHT);
	
	SetDrawTPage(&buffer->logoRightTPage,0,0,GetTPage(1,0,LOGO_TEXTURE_X_POS+160,0));
	SetSprt(&buffer->logoRightSprite);
	setUV0(&buffer->logoRightSprite, 0, 0);
	setWH(&buffer->logoRightSprite,64,LOGO_TEXTURE_HEIGHT);
	setXY0(&buffer->logoRightSprite,256,0);
	SetShadeTex(&buffer->logoRightSprite, 1); // Set texture shading to off
	buffer->logoRightSprite.clut = GetClut(LOGO_TEXTURE_X_POS,LOGO_TEXTURE_HEIGHT);
...
void addLandPolys(u_long *ot,POLY_F3 *polyList)
{
	...
	for( y=LAND_NUM_VERT_CELLS-2; y>0; y--)
	{
		for( x=0; x<LAND_NUM_HORIZ_CELLS-1; x++)
		{
			...
			SVECTOR *v3 = &cells[y][x+1];
			SVECTOR *v4 = &cells[y-1][x+1];
			SVECTOR *v5 = &cells[y-1][x];
